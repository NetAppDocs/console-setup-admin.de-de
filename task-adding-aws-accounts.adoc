...
summary: Fügen Sie AWS-Anmeldeinformationen hinzu und verwalten Sie diese, damit die NetApp Console über die erforderlichen Berechtigungen zum Bereitstellen und Verwalten von Cloud-Ressourcen in Ihren AWS-Konten verfügt.  Wenn Sie mehrere AWS-Abonnements verwalten, können Sie jedem von ihnen auf der Seite „Anmeldeinformationen“ unterschiedliche AWS-Anmeldeinformationen zuweisen. 
---
...
+
Durch das Hinzufügen neuer AWS-Anmeldeinformationen zur Konsole erhalten Sie die erforderlichen Berechtigungen zum Erstellen eines Konsolenagenten. <<add-aws-credentials-agent-creation,Erfahren Sie, wie Sie AWS-Anmeldeinformationen zur NetApp Console hinzufügen>> .
...
link:concept-accounts-aws.html["Erfahren Sie, wie die NetApp Console AWS-Anmeldeinformationen und -Berechtigungen verwendet"].
...
Wenn Sie der NetApp Console AWS-Schlüssel für einen IAM-Benutzer bereitstellen möchten, müssen Sie diesem Benutzer die erforderlichen Berechtigungen erteilen. Die IAM-Richtlinie der NetApp Console definiert die AWS-Aktionen und -Ressourcen, die die NetApp Console verwenden darf.
...
NOTE: Es kann einige Minuten dauern, bis neue Anmeldeinformationen bei Ihrem Cloud-Anbieter verfügbar sind.
...
Richten Sie eine IAM-Rolle ein, die es der NetApp Console Software-as-a-Service (SaaS)-Anwendung ermöglicht, die Rolle zu übernehmen.
...
----
+
Weitere Informationen zum kontoübergreifenden Ressourcenzugriff in IAM finden Sie unter link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["AWS Identity and Access Management (IAM)-Dokumentation"^].
...
.Ergebnis
Die IAM-Rolle verfügt jetzt über die erforderlichen Berechtigungen. <<add-the-credentials,Sie können es jetzt zur Konsole hinzufügen>>.
...
.Bevor Sie beginnen
Wenn Sie die IAM-Rolle gerade erst erstellt haben, kann es einige Minuten dauern, bis sie einsatzbereit ist. Warten Sie einige Minuten, bevor Sie die Anmeldeinformationen zur NetApp Console hinzufügen.
...
. Wählen Sie auf der Seite *Anmeldeinformationen der Organisation* die Option *Anmeldeinformationen hinzufügen* aus und folgen Sie den Schritten des Assistenten.
+
.. *Speicherort der Anmeldeinformationen*: Wählen Sie *Amazon Web Services > Konsole*.
...
Für Einzelheiten siehe die https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Konsolendokumentation für Amazon FSx für ONTAP"^]
...
.Bevor Sie beginnen
Sie müssen einen Konsolenagenten erstellen, bevor Sie ein Abonnement konfigurieren können. link:concept-agent-install-options.html["Erfahren Sie, wie Sie einen Konsolenagenten erstellen"].
...
Wenn Sie ein Abonnement über den AWS Marketplace abschließen, besteht der letzte Schritt darin, das Abonnement Ihrer Organisation zuzuordnen. Falls Sie diesen Schritt nicht ausgeführt haben, können Sie das Abonnement nicht für Ihre Organisation nutzen.
...
.Schritte
. Bestätigen Sie, dass Sie Ihr Abonnement nicht mit Ihrer Konsolenorganisation verknüpft haben.
+
.. Wählen Sie im Navigationsmenü *Verwaltung > Licenses and subscriptions*.
...
. Bestätigen Sie, dass das Abonnement mit Ihrer Organisation verknüpft ist.
+
...
.Schritte
. Wählen Sie *Administration > Anmeldeinformationen*.
. Wählen Sie auf der Seite *Anmeldeinformationen der Organisation* das Aktionsmenü für einen Satz von Anmeldeinformationen aus und wählen Sie dann *Anmeldeinformationen bearbeiten*.
...
TIP: Sie können die Anmeldeinformationen für ein Instanzprofil, das einem Konsolenagenten zugeordnet ist, nicht löschen.
