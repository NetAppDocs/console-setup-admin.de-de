...
summary: Sie können einen Console-Agenten in AWS direkt über die NetApp Console bereitstellen. Vor der Bereitstellung eines Console-Agenten in AWS über die Console müssen Sie Ihr Netzwerk einrichten und die AWS-Berechtigungen vorbereiten. 
---
= Einen Console-Agenten in AWS über die NetApp Console bereitstellen
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Sie können einen Console-Agenten in AWS direkt über die NetApp Console bereitstellen. Vor der Bereitstellung eines Console-Agenten in AWS über die Console müssen Sie Ihr Netzwerk einrichten und die AWS-Berechtigungen vorbereiten.
...
+
Diese Richtlinie enthält nur die Berechtigungen, die zum Starten des Konsolenagenten in AWS von der Konsole aus erforderlich sind.  Wenn die Konsole den Konsolenagenten erstellt, wendet sie einen neuen Satz von Berechtigungen auf den Konsolenagenten an, der es dem Konsolenagenten ermöglicht, AWS-Ressourcen zu verwalten. link:reference-permissions-aws.html["Anzeigen der für den Konsolenagenten selbst erforderlichen Berechtigungen"].
...
Erstellen Sie den Konsolenagenten direkt über die webbasierte Konsole.
...
. Wählen Sie auf der Seite *Übersicht* die Option *Agent bereitstellen > AWS*.
. Befolgen Sie die Schritte im Assistenten, um den Konsolenagenten zu erstellen:
. Auf der Seite *Einführung* sehen Sie die Übersicht über den Prozess.
. Geben Sie auf der Seite *AWS-Anmeldeinformationen* Ihre AWS-Region an und wählen Sie dann ein Authentifizierungsverfahren. Dies kann entweder eine IAM-Rolle sein, die die Console annehmen kann, oder ein AWS-Zugriffsschlüssel und ein geheimer Schlüssel.
+
...
. Auf der Seite *Netzwerk* können Sie eine VPC, ein Subnetz und ein Schlüsselpaar für den Agenten angeben, auswählen, ob eine öffentliche IP-Adresse aktiviert werden soll, und optional eine Proxy-Konfiguration festlegen.
+
