...
. Sie müssen einen Konsolen-Agenten mit einem Google-Konto bereitstellen, das über die Berechtigung zum Starten des Konsolen-Agenten von der Konsole aus verfügt.
. Bei der Bereitstellung des Console-Agenten werden Sie aufgefordert, ein  https://cloud.google.com/iam/docs/service-accounts["Dienstkonto"^] für den Agenten auszuwählen. Die Console erhält Berechtigungen vom Dienstkonto, um Cloud Volumes ONTAP-Systeme zu erstellen und zu verwalten, Backups mithilfe von NetApp Backup und Recovery zu verwalten und mehr. Die Berechtigungen werden durch das Anhängen einer benutzerdefinierten Rolle an das Dienstkonto bereitgestellt.
