...
---
= Manuelle Bereitstellung eines Console Agenten in AWS für NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Sie können einen Console-Agenten manuell auf einem Linux-Host in AWS bereitstellen. Um den Console-Agenten manuell auf Ihrem eigenen Linux-Host bereitzustellen, müssen Sie die Host-Anforderungen prüfen, Ihr Netzwerk einrichten, die AWS-Berechtigungen vorbereiten, den Console-Agenten bereitstellen und anschließend die vorbereiteten Berechtigungen erteilen.

Durch die Bereitstellung eines Agenten in AWS wird eine EC2-Instanz gestartet, auf der Linux und die Console-Agent-Software in einer VPC Ihrer Wahl ausgeführt werden.

.Bevor Sie beginnen
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["Erfahren Sie, wann Sie in AWS manuell bereitstellen sollten"].
...
PUT-Antwort-Hop-Limit bei Verwendung von IMDSv2:: Wenn IMDSv2 aktiviert ist (Standardeinstellung für neue EC2-Instanzen), setzen Sie das Hop-Limit für PUT-Antworten auf 3.  Andernfalls wird während der Agenteneinrichtung ein UI-Initialisierungsfehler angezeigt.
+
--
...
+
Je nach den NetApp Datendiensten, die Sie verwenden möchten, müssen Sie möglicherweise eine zweite Richtlinie erstellen. In Standardregionen werden die Berechtigungen auf zwei Richtlinien verteilt. Zwei Richtlinien sind aufgrund einer maximalen Zeichenlängenbeschränkung für verwaltete Richtlinien in AWS erforderlich. link:reference-permissions-aws.html["Weitere Informationen zu IAM-Richtlinien für den Konsolenagenten"].
...
Wenn Sie Amazon S3-Buckets im selben AWS-Konto haben, in dem Sie den Console-Agenten erstellt haben, wird automatisch ein Amazon S3-Speichersystem auf der Seite *Systeme* angezeigt. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["Erfahren Sie, wie Sie S3-Buckets über die NetApp Console verwalten"^]
