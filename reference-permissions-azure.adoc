...
=== Zusätzliche Berechtigungen für Datensicherung und -wiederherstellung
...
Der Agent führt die folgenden API-Anfragen aus, wenn Sie die Such- und Wiederherstellungsfunktion verwenden. Diese Berechtigungen benötigen Sie nur, wenn Sie vor der Veröffentlichung von Indexing v2 ältere Indexierungsfunktionen aktiviert haben:
...
Die folgenden Berechtigungen wurden aus dem JSON-Rollenbeispiel entfernt, damit das Beispiel die minimal erforderlichen Berechtigungen für Backup und Recovery widerspiegelt:
...
====
Dem JSON-Rollenbeispiel wurden die folgenden Berechtigungen hinzugefügt:
