...
Wenn Sie einen Konsolenagenten über die Konsole bereitstellen, müssen Sie die ARN einer IAM-Rolle oder Zugriffsschlüssel für einen IAM-Benutzer angeben. Die Authentifizierungsmethode muss über Berechtigungen zum Bereitstellen des Console-Agenten in AWS verfügen.  Die erforderlichen Berechtigungen sind in der folgenden Liste aufgeführt:link:task-install-agent-aws-console.html#aws-permissions-agent["Agentenbereitstellungsrichtlinie für AWS"] Die
