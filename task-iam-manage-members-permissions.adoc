...
Sie können eine föderierte Gruppe von Ihrem Identitätsanbieter (IdP) zu Ihrer Organisation hinzufügen. Die Mitglieder der föderierten Gruppe erben die Rollen, die Sie der Gruppe in der Konsole zuweisen.

.Bevor Sie beginnen:
Stellen Sie vor dem Hinzufügen einer föderierten Gruppe Folgendes sicher: * Sie müssen bereits eine Föderation zwischen Ihrem IdP und der Konsole eingerichtet haben. link:concept-federation.html["Erfahren Sie, wie Sie eine Föderation einrichten."] * Die Gruppe muss bereits in Ihrem IdP existieren. * Sie müssen den genauen Namen der Gruppe in Ihrem IdP kennen. * Benutzer, die dieser Gruppe angehören, müssen sich bereits für die NetApp Console registriert haben und ihnen muss explizit eine Rolle in der Console zugewiesen worden sein.
