...
Sie können Ihrer Organisation zwei Arten von Mitgliedern hinzufügen: ein Benutzerkonto und ein Dienstkonto. Anwendungen nutzen Dienstkonten, um API-Aufgaben automatisch auszuführen.  Benutzerkonten werden verwendet, um sich anzumelden und Ressourcen zu verwalten.
...
Wenn Sie ein Mitglied entfernen, entzieht das System ihm die Berechtigungen, behält aber seine Console- und NetApp -Support-Site-Konten bei.
...
. Laden Sie die Client-ID und das Client-Geheimnis herunter oder kopieren Sie sie. + Die Konsole zeigt das Client-Geheimnis nur einmal an. Kopieren oder laden Sie es herunter und speichern Sie es sicher.
...
Nach der Deaktivierung müssen die Nutzer die Multi-Faktor-Authentifizierung (MFA) bei der nächsten Anmeldung erneut einrichten. Wenn der Benutzer den Zugriff auf sein MFA-Gerät nur vorübergehend verloren hat, kann er zur Anmeldung den Wiederherstellungscode verwenden, den er beim Einrichten von MFA gespeichert hat.
