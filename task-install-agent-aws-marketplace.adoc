...
---
= Stellen Sie einen Konsolenagenten aus dem AWS Marketplace bereit.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Sie stellen einen Console-Agenten in AWS direkt über den AWS Marketplace bereit. Um einen Console-Agenten aus dem AWS Marketplace bereitzustellen, müssen Sie Ihre Netzwerkkonfiguration einrichten, die AWS-Berechtigungen vorbereiten, die Instanzanforderungen prüfen und anschließend den Console-Agenten bereitstellen.
...
.Bevor Sie beginnen
* link:concept-agent-install-options.html#agent-marketplace-deploy["Erfahren Sie, wann Sie über den AWS Marketplace bereitstellen sollten"].
