...
summary: Installieren Sie einen Console-Agenten lokal, melden Sie sich anschließend an und richten Sie ihn für die Zusammenarbeit mit Ihrer Console-Organisation ein. 
---
= Manuelle Installation eines NetApp Console-Agenten vor Ort
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Installieren Sie einen Console-Agenten lokal, melden Sie sich anschließend an und richten Sie ihn für die Zusammenarbeit mit Ihrer Console-Organisation ein.


NOTE: Wenn Sie ein VMware-Benutzer sind, können Sie eine OVA verwenden, um einen Console-Agenten in Ihrem vCenter bereitzustellen. link:task-install-agent-on-prem-ova.html["Erfahren Sie mehr über die Bereitstellung eines Agenten in einer vCenter."]

Vor der Installation müssen Sie sicherstellen, dass Ihr Host (VM oder Linux-Host) die Anforderungen erfüllt und dass der Konsolenagent ausgehenden Zugriff auf das Internet sowie auf die Zielnetzwerke hat. Wenn Sie NetApp Datenservices oder Cloud-Speicheroptionen wie Cloud Volumes ONTAP nutzen möchten, müssen Sie bei Ihrem Cloud-Anbieter Anmeldeinformationen erstellen, um diese der Konsole hinzuzufügen, damit der Konsolenagent in Ihrem Namen Aktionen in der Cloud ausführen kann.
...
Führen Sie den Console-Agent auf einem x86-Host aus, der die Anforderungen an Betriebssystem, RAM und Ports erfüllt. Stellen Sie sicher, dass Ihr Host diese Anforderungen erfüllt, bevor Sie den Console-Agent installieren.
...
Wenn Sie NetApp Datendienste in AWS oder Azure mit einem lokalen Konsolenagenten nutzen möchten, müssen Sie Berechtigungen bei Ihrem Cloud-Anbieter einrichten und anschließend die Anmeldeinformationen zum Konsolenagenten hinzufügen, nachdem Sie diesen installiert haben.
...
Wenn Sie einen Console-Agent manuell installieren, müssen Sie Ihre Systemumgebung so vorbereiten, dass sie die Anforderungen erfüllt. Sie benötigen eine Linux-Maschine und müssen je nach Ihrem Linux-Betriebssystem Podman oder Docker installieren.
...
. Wenn dieser Agent mit einem Cloud Volumes ONTAP System verknüpft wird, kann eine Erhöhung der dem Agenten zugeordneten Thread-Anzahl die Leistung verbessern. link:https://kb.netapp.com/Cloud/ncds/nds/cvods/cvods_kbs/CVO_going_to_failed_state_frequently_in_NetApp_Console["Informationen zur Erhöhung des Thread-Limits des Agentencontainers"^].
