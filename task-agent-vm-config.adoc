...
summary: Sie können Änderungen an Ihrem bestehenden vCenter oder ESXi-Host vornehmen, nachdem Sie den Konsolenagenten bereitgestellt haben. Beispielsweise können Sie die CPU oder den Arbeitsspeicher der VM-Instanz erhöhen, die den Konsolenagenten hostet. 
---
= Halten Sie einen vCenter oder ESXi-Host für den Konsolenagenten bereit.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Sie können Änderungen an Ihrem bestehenden vCenter oder ESXi-Host vornehmen, nachdem Sie den Konsolenagenten bereitgestellt haben. Beispielsweise können Sie die CPU oder den Arbeitsspeicher der VM-Instanz erhöhen, die den Konsolenagenten hostet.
...
Sie können über den vSphere-Client auf die Wartungskonsole zugreifen.
...
Bearbeiten Sie die VM-Instanz-Einstellungen in Ihrem vCenter oder ESXi-Host und verwenden Sie anschließend die Wartungskonsole, um die Änderungen anzuwenden.
...
Überprüfen Sie die Netzwerkeinstellungen der Agenten-VM im vSphere Client, um Netzwerkprobleme zu bestätigen oder zu beheben. Sie können lediglich die folgenden Netzwerkeinstellungen anzeigen (nicht aktualisieren): IP-Adresse und DNS-Details.
