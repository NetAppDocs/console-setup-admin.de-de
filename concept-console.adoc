...
summary: Die NetApp Console bietet eine SaaS-Lösung für den vollen Funktionsumfang oder eine selbstgehostete Option für Organisationen mit eingeschränkter Konnektivität. 
---
...
[role="lead"]
Die NetApp Console bietet eine SaaS-Lösung für den vollen Funktionsumfang oder eine selbstgehostete Option für Organisationen mit eingeschränkter Konnektivität.

Sie können die NetApp Console auf zwei Arten nutzen: als vollständig verwaltete SaaS-Lösung oder als selbstgehostete Lösung in Ihrer eigenen Cloud-Umgebung. Bei der SaaS-Option greifen Sie über console.netapp.com auf die Konsole zu, während NetApp die gesamte Infrastruktur, Authentifizierung und Aktualisierungen verwaltet. Sie erhalten direkten Zugriff auf alle Funktionen über Ihren Webbrowser. Wenn Sie sich für die selbstgehostete Option entscheiden, installieren Sie einen Console-Agenten in Ihrer AWS-, Azure- oder Google Cloud-Umgebung. Die Schnittstelle läuft lokal, und der Agent hält eine eingeschränkte ausgehende Verbindung zu NetApp für wichtige Aufgaben wie Authentifizierung und Aktualisierungen aufrecht.

Wählen Sie die SaaS-Lösung, wenn Ihre Organisation über eine Standard-Internetverbindung verfügt. Sie profitieren von einfachster Einrichtung, automatischen Updates und vollem Zugriff auf alle NetApp -Speichersysteme, Datendienste und erweiterten Funktionen. Sie können Agenten direkt über die Konsole hinzufügen, und NetApp übernimmt die gesamte Wartung.

Wählen Sie die Option „Selbstgehostet“, wenn Ihre Organisation strenge Anforderungen an Konnektivität, Sicherheit oder Compliance stellt, beispielsweise in der öffentlichen Verwaltung oder in regulierten Branchen. Sie erhalten mehr Kontrolle und Netzwerkisolation, können aber nur ausgewählte NetApp -Speichersysteme verwalten und nur eingeschränkte Datendienste nutzen, wobei einige Funktionen nicht verfügbar sind. Sie installieren einen Konsolenagenten manuell und greifen direkt über die lokale Agentenschnittstelle auf die Konsole zu. Installieren Sie Ihren Agenten über Ihren Cloud-Marktplatz oder per manueller Installation. Der Agent kommuniziert ausschließlich ausgehend mit NetApp, um sicherzustellen, dass keine eingehenden Internetverbindungen eingehen.
...
image:diagram-standard-mode.png["Eine konzeptionelle Abbildung, die die Architektur des Standardmodus mit dem öffentlichen Internet zeigt, in dem die SaaS-Schicht und die Auth0-Authentifizierung verfügbar sind."]
...
Erste Schritte mit dem Standardmodus:: Gehen Sie zum https://console.netapp.com["NetApp Console"^] und melden Sie sich an.
+
--
...
image:diagram-restricted-mode.png["Eine konzeptionelle Darstellung des eingeschränkten Modus."]
...
--
Erste Schritte mit dem eingeschränkten Modus:: Sie müssen den eingeschränkten Modus aktivieren, wenn Sie Ihre NetApp Console Konsolenorganisation erstellen.
