...
---
= NetApp Console mit Active Directory Federation Services föderieren
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Sie können eine Verbundauthentifizierung mit Ihrem AD FS-Server einrichten, um Single Sign-On (SSO) für NetApp Console zu aktivieren. Dazu müssen Sie Ihren AD FS so konfigurieren, dass er der Console als Dienstanbieter vertraut, und anschließend die Verbindung in der NetApp Console herstellen.
...
+
.. Erstellen Sie die Vertrauensstellung mit PowerShell, indem Sie das folgende Skript ausführen:
+
[source, powershell]
----
...
+
*** Verwenden Sie beim Erstellen des Relying Trust Identifier den Wert *YOUR_TENANT*: `netapp-cloud-account`
