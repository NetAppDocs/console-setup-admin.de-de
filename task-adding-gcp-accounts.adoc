...
---
= Verknüpfen Sie die Anmeldeinformationen des Konsolenagenten mit Ihrem Google Cloud-Abonnement
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Wenn Sie einen Konsolenagenten in Google Cloud erstellen, können Sie die zugehörigen Anmeldeinformationen verwenden, um NetApp Intelligent Services zu abonnieren, zu denen Cloud Volumes ONTAP und NetApp -Datendienste gehören.  Sie müssen das Abonnement mit dem Agenten verknüpfen, dessen Anmeldeinformationen Sie zum Bezahlen der Dienste verwenden möchten.
...
Durch das Ersetzen des aktuellen Marktplatzabonnements durch ein neues wird das Abonnement für vorhandene und neue Cloud Volumes ONTAP -Systeme aktualisiert.
...
NOTE: Wenn jemand aus Ihrer Organisation bereits ein Marktplatz-Abonnement von Ihrem Abrechnungskonto hat, werden Sie weitergeleitet zu https://bluexp.netapp.com/ontap-cloud?x-gcp-marketplace-token=["die Cloud Volumes ONTAP -Seite in der NetApp Console"^] stattdessen.  Wenn dies unerwartet vorkommt, wenden Sie sich an Ihr NetApp -Vertriebsteam.  Google ermöglicht nur ein Abonnement pro Google-Abrechnungskonto.
...
Das Abonnieren von NetApp -Datendiensten über den Google Cloud Marketplace kann aufgrund falscher Berechtigungen oder einer fehlenden Weiterleitung zur Konsole fehlschlagen. Führen Sie in diesem Fall die folgenden Schritte aus, um den Abonnementvorgang abzuschließen.
