...
== Überprüfen Sie den Netzwerk- und Portzugriff für den Console-Agenten
...
Führen Sie Konfigurationsprüfungen an den Console Agents über die Console oder die Agent Maintenance Console durch, um sicherzustellen, dass sie auf die erforderlichen Endpunkte zugreifen können und die erforderlichen Ports geöffnet sind.

Sie können Konfigurationsprüfungen auch über die Agentenwartungskonsole durchführen.link:reference-agent-maint-console-config-checker.html["Erfahren Sie mehr über die Verwendung des Befehls config-checker validate."]
