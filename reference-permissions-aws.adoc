...
.14+| Erstellen und verwalten Sie IAM-Rollen und Instanzprofile für Cloud Volumes ONTAP -Instanzen | iam:ListInstanceProfiles | Ja | Ja | Nein 
...
.6+| Verwalten Sie EBS-Volumes, die Cloud Volumes ONTAP als Backend-Speicher verwendet | ec2:CreateVolume | Ja | Ja | Nein 
...
.3+| Aktivieren Sie die Datenverschlüsselung von Cloud Volumes ONTAP mithilfe des AWS Key Management Service (KMS). | kms:Neuverschlüsseln* | Ja | Nein | Nein 
...
Für die Data Classification ist nun folgende Berechtigung erforderlich:
