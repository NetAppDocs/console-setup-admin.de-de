...
summary: Mit der Wartungskonsole des Console-Agenten können Sie die Installation und Konfiguration eines Console-Agenten überprüfen. 
---
...
Sie können die Wartungskonsole vom Host des Konsolenagenten aus aufrufen. Navigieren Sie zum folgenden Verzeichnis:
...
`--validationTypes <comma-separated list of validation types to run>` --*ERFORDERLICH*-- Wählen Sie einen oder mehrere Validierungstypen aus, die ausgeführt werden sollen. Gültige Validierungstypen sind:
...
Die folgenden Beispiele umfassen die Überprüfung der Netzwerkverbindungen, der erforderlichen Ports und der Hardwareanforderungen sowie verschiedener Kombinationen aus der Verwendung eines Proxyserver und benutzerdefinierter Zertifikate.
