...
Standardmäßig werden Benutzer nicht zur Eingabe ihrer ONTAP-Anmeldeinformationen aufgefordert, um auf ONTAP System Manager zuzugreifen. Ein Console-Administrator (mit der Rolle Organization admin) kann die Console jedoch so konfigurieren, dass Benutzer zur Eingabe ihrer ONTAP-Anmeldeinformationen aufgefordert werden. Wenn diese Einstellung aktiviert ist, müssen Benutzer ihre ONTAP-Anmeldeinformationen jedes Mal eingeben.
